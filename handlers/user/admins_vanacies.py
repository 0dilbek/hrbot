...
from keyboards.reply import kasblar_lst_btn, working_time_btn, back_btn, main_menu_users_btn, vacancies_btn, confirm_btn, experience_btn
...
async def admins_vanacies_foreign_language(message: Message, state: FSMContext):
    ...
    await message.answer("Xorijiy tilni bilish darajangizni kiriting.", reply_markup=back_btn)
...
async def admins_vanacies_foreign_language_level(message: Message, state: FSMContext):
    ...
    foreign_languages.append({
        'language': state_data.get('foreign_language', ''),
        'level': message.text
    })
...
async def admins_vanacies_continue_foreign_language(message: Message, state: FSMContext):
    if message.text == "Ha":
...
async def admins_vanacies_why_leave_work(message: Message, state: FSMContext):
    ...
    await message.answer("Oxirgi ishlagan tashkilotingiz yoki to‘g‘ridan-to‘g‘ri rahbaringiz telefon raqamini qoldiring.\n\nMasalan: +998901234567", reply_markup=back_btn)
    await state.set_state(AdminsVacancyState.last_work_place_phone)
...
Nega aynan bizni tanladingiz?: {state_data.get("why_choice_us")}
        ...
        for admin in ADMINS:
            try:
                if user.profile_pic_file_id:
                    await message.bot.send_photo(chat_id=admin, photo=user.profile_pic_file_id, caption=caption, parse_mode="HTML")
                else:
                    await message.bot.send_message(chat_id=admin, text=caption, parse_mode="HTML")
        ...
        if vacancy_text and len(vacancy_text.last_text.strip()) > 5:
            await message.answer(vacancy_text.last_text, parse_mode="HTML", reply_markup=main_menu_users_btn(is_registered=True))
        else:

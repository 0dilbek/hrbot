...
from aiogram.types import Message, PollAnswer, ReplyKeyboardRemove, CallbackQuery, FSInputFile
...
async def select_position(callback_query: CallbackQuery, state: FSMContext):
    ...
    if vacansy_text and vacansy_text.text.strip():
        await callback_query.message.answer(vacansy_text.text.strip(), parse_mode="HTML")
...
async def confirm(message: Message, state: FSMContext):
    if message.text.lower() == "ha":
        ...
        sertificates_text = "\n\n"
        if state_data.get('sertificates'):
            sertificates_text += "\n".join([f"\t\t• {ser['name']} - {ser['ball']}" for ser in state_data.get('sertificates', [])]) + "\n"
        phones_text = "\n\n"
